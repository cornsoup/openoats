...
/// (rename, new folder).
///
/// Extracted from NotesView so both UnifiedOpenOatsView and the (soon-to-be-
/// removed) NotesView can host the same sidebar UI without duplication.
struct NotesSidebarView: View {
    ...
    // MARK: - New folder sheet state (session-level only)
        ...
        VStack(spacing: 0) {
            ...
            if bulkDeleteMode {
                ...
                let selectedBinding = Binding<String?>(
                    ...
                    set: { newID in
                        // When a recording is in progress, single-clicking a past session
                        // opens it in a new window rather than replacing the live detail area.
                        if let id = newID, coordinator.liveSessionController?.state.isRunning == true {
                            openWindow(id: "meeting", value: id)
                        } else {
                            controller.selectSession(newID)
                        }
                    }
                )
    ...
    // MARK: - New folder sheet (session-level)
    ...
    // MARK: - Folder color / definition helpers
